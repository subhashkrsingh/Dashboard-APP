...
const CACHE_KEY = "oil-gas-sector-snapshot";
const CACHE_TTL_SECONDS = Math.max(Number(process.env.OIL_GAS_SECTOR_CACHE_TTL_SEC) || 20, 10);
const SNAPSHOT_PATH = path.join(__dirname, "..", "data", "oilGasSectorSnapshot.json");
...
function loadBundledSnapshot() {
  ...
  try {
    ...
    console.warn("Unable to load bundled oil & gas sector snapshot:", error?.message || error);
    return null;
  }
}

function setOilGasSectorCache(snapshot) {
...
function setOilGasSectorRefreshError(error) {
  lastRefreshError = {
...
function getFreshOilGasSectorCache() {
...
function getLastSuccessfulOilGasSectorSnapshot() {
...
function getBundledOilGasSectorSnapshot() {
  return loadBundledSnapshot();
}

function getOilGasSectorStatus() {
  return {
...
module.exports = {
  ...
  getOilGasSectorStatus,
  getLastSuccessfulOilGasSectorSnapshot,
  setOilGasSectorCache,
  setOilGasSectorRefreshError
};
